...
# 10/05/2025
# ------------------------------------------------------------------------------
setwd("C:/Users/larag/Documents/Master bioinfo/TFM/htseq_counts/htseq_counts")
...
ruta_archivos <- "C:/Users/larag/Documents/Master bioinfo/TFM/htseq_counts/htseq_counts"
...
# Ahora usamos ese vector para renombrar los archivos
...
# Aparecían mal asignados los nombres de los genes, ordenando el objeto ann ya está
...
# 8. Test de expresión diferencial (QLF)
res <- glmQLFTest(fit, contrast = contraste)
# likelihook ratio test (DGELRT)
head(res$table)
dim(res$table)

# 9. Corrección de datos positivos, clasifica los genes de más a menos significativos
res_corrected <- topTags(res, n = Inf) # ponemos infinito para que reporte todos los genes
dim(res_corrected)
head(res_corrected)

# 10. Expresión diferencial de genes
...
# añadir una columna nueva a data y la modifica para que aparezca up y down segun 
...
ggplot(data, aes(x = logFC, y = -log10(FDR), col = DE)) +
  ...
  geom_text_repel(data = subset(data, FDR < 0.05 & abs(logFC) > 1.5), 
                  ...
                  size = 3,
                  max.overlaps = 20) +
  theme_classic() #theme(aspect.ratio = 1, plot.margin = margin(20, 20, 20, 20))
